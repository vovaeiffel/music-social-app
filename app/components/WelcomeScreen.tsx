...
export default function WelcomeScreen({ onGoogleLogin }: Props) {
  return (
    <div className="min-h-screen w-full bg-zinc-950 flex flex-col items-center justify-center p-6 text-white relative overflow-hidden">
      {/* Фоновое изображение (ваша картинка на весь экран) */}
      <div
        className="absolute inset-0 bg-cover bg-center opacity-20 pointer-events-none"
        style={{ backgroundImage: "url('/logo.png')" }}
      />

      {/* Затемнение поверх картинки, чтобы текст и кнопка легко читались */}
      <div className="absolute inset-0 bg-zinc-950/80 backdrop-blur-sm pointer-events-none" />

      {/* Основной блок по центру */}
      <div className="w-full max-w-sm flex flex-col items-center text-center z-10 relative">
        {/* Аккуратный логотип сверху текста */}
        <div className="w-24 h-24 mb-6 relative flex items-center justify-center">
          <img
            src="/logo.png"
            alt="App Logo"
            className="w-full h-full object-contain rounded-2xl border border-white/10 shadow-2xl shadow-black/50"
            // Если лого чёрное на белом и сливается, раскомментируйте строчку ниже:
